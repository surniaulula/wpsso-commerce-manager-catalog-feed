...
if ( ! class_exists( 'WpssoFcfSubmenuFcfGeneral' ) && class_exists( 'WpssoAdmin' ) ) {

	class WpssoFcfSubmenuFcfGeneral extends WpssoAdmin {
		...
		protected function add_meta_boxes() {
			...
			$metabox_id      = 'fcf';
			$metabox_title   = _x( 'Facebook Catalog Feed XML', 'metabox title', 'wpsso-facebook-catalog-feed' );
		...
		public function show_metabox_fcf() {
			...
			switch ( $metabox_id . '-' . $tab_key ) {
				...
				case 'fcf-general':
					...
					foreach ( $locale_names as $locale => $native_name ) {
						...
						$url  = WpssoFcfRewrite::get_url( $locale );

						$table_rows[ 'fcf_url_' . $locale ] = '' .
